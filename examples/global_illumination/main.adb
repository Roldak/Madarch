...
procedure Main is
   ...
   Window : Windows.Window := Windows.Open (1000, 1000, "Global_Illumination");
...
begin
   ...
   Renderers.Set_Material
     (Renderer, 5, Materials.Create ((0.0, 1.0, 0.0), 0.8, 0.3));
   ...
   while Window.Is_Opened loop
      ...
      Window.Poll_Events;
      Time := Time + 0.01;
      Spot_Light_Instance.Set
        (Lights.Spot_Lights.Direction,
         Values.Vector3 ((Cos (Time), Sin (Time), 0.0)));
      Renderers.Set_Light (Renderer, 1, Lights.Spot_Lights.Spot_Light, Spot_Light_Instance);
   end loop;
end Main;
